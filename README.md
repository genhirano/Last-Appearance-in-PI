...
* 例えば
  * 自然数「999999」（ゼロが6個連続）は762桁目に出現します。  
  * 自然数「12345678」は円周率の少数以下186,557,266桁目に出現します。	
...
## アプローチ
円周率は予測不能ですので、すべての自然数が円周率に含まれていることを証明するためには、**ひとつづず総当たりで探す**しかありません。  
自然数は無限にありますので、桁数毎に区切って検索します。
...
膨大な検索量ですが、「円周率の中には、この世の中のすべての自然数が含まれている」のであれば、すべて発見できるはずです。。。でも目視ではとても難しそうです。。。

**対象桁において一番遅く出現する自然数が特定できれば、対象桁の自然数はすべて出現したと言えますので、プログラムを作成して実行します。**
...
### プログラムの方針
**自然数を桁数ごとにひとつづつ円周率データの中から検索し、桁数ごとに一番最後に出現する自然数を特定する。** 
* 円周率のデータ
...
* アルゴリズム
  * 桁数毎にすべての自然数を検索し、その桁数の自然数で一番最後（一番深い位置）に出現した自然数を一つ特定する。
  * 自然数は対象桁で左ゼロパディングする。
  * 1桁から開始し、制限なくN桁まで実行する。
* 膨大なデータに対応すること
  * 円周率データも自然数も（おそらく）無限であるため、富豪的なアルゴリズムを採用しない。
* 中断に寛容であること
...
* 非富豪的アプローチ
  ...
  * YCD形式ファイルは時に膨大なファイルサイズになります。非力なコンピュータでは、このファイルを一度にメモリにロードできません。このプログラムでは、非力なコンピュータでも実行可能なように、YCDファイルを一度にロードせず、少しづつ切り出してストレージから取り出せるようになっています。

* 組み込みWEBサーバーと静的HTMLファイルコンパイル
  * プログラムにはWEBサーバーが組み込まれています。プログラムを開始するだけでWEBサーバーが起動し、WEB画面により計算中の様子が確認できます。
  * WEB画面を表示するたびに、平行して画面はHTML静的コンパイルします。このHTMLファイルを使って例えばFTPで公開用レンタルサーバーへアップロードするなど活用できます。 
