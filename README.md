...
> [!NOTE]
> Raspberry Pi のような非力なマシンでの動作を前提に、YCDファイルはヘッダー位置へシークした後に8バイト(19桁)単位で順次読み込みます。  
> そのため 1ファイル60GB級・複数ファイル構成でも、ファイル全体をメモリ展開せずに処理できます（メモリ使用量は主に `unitLength` と検索対象桁数に依存）。
