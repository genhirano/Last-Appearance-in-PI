...
円周率を3.1415....とスキャンしていくと、小数点以下762桁目から6個の“9”の並びを見つけることができます。”9”が6個も並ぶのはとても珍しい数字ですが、円周率の中には確かに存在しています。

なお、”9”が6個並ぶ円周率小数点以下762桁目は「Feynman point」（ファインマン・ポイント）と呼ばれており、お茶目で優秀な物理学者であるリチャード・P・ファインマンがこの桁まで縁数率を暗唱したことで有名です。
ファインマン・ポイント - Wikipedia : https://ja.wikipedia.org/wiki/ファインマン・ポイント

このように、珍しい数字（の並び）であっても円周率のどこかに存在しているはずらしいのです。

さて、珍しい数字とは何でしょうか。
例えば、
”111111”は少数以下255,945桁目、
"1234567"は少数以下9,470,344桁目、
のような珍しい数字も、検索深度を深めれば見つかるようです。

再度、珍しい数字とは何でしょうか。
私たちの感覚では、同じ数字が続いたり、車のナンバーだったり誕生日だったりしますが、私たち基準ではなく、円周率の中で珍しい数字とはいったいどの数字なのでしょうか。円周率の中のレアな数字とは。

このプロジェクトは、円周率「3.1415...」をスキャンしたときに最も出現が遅い数字（数列）を円周率の中で最も珍しい数字であると定義します。
つまり、1桁であれば”0”、2桁であれば”68”が最も珍しい数字です。

さらには4桁、5桁と探索の桁数を伸ばし、最も遅く出現する数字を見つけることを目的としています。
