...
* 例えば
  * 整数列「999999」（ゼロが6個連続）は762桁目に出現します。  
  * 整数列「12345678」は円周率の少数以下186,557,266桁目に出現します。	
...
> [NOTE]
> 整数列「999999」（ゼロが6個連続）が出現する762桁目は「ファインマン・ポイント」[(→Wikipedia)](https://ja.wikipedia.org/wiki/ファインマン・ポイント)と呼ばれています。
...
**各桁ごとに、出現が一番遅かった整数列を特定することができれば、** その桁の整数列はすべて出現したと言えます。  
これは目視では（少なくとも私には）難しいので、プログラムを作成して実行します。
> [NOTE]
>そのプログラムソースコードがこのリポジトリに格納されています。

### プログラムの方針
「この世の中のあらゆるすべての整数列」を、**桁数ごとにひとつづつ円周率データの中から検索し、一番最後に出現した整数列を特定する。** 
* 円周率のデータ
...
* アルゴリズム
  * 桁数毎にすべての整数列を検索し、その桁数の整数列で一番最後（一番深い位置）に出現した整数列を一つ特定する。
  * 整数列は左ゼロパディングする。
