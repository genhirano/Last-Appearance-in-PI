...
## PROJECT OVERVIEW
「円周率の中には、この世の中のあらゆるすべての整数列が含まれている」と言われています。

本当でしょうか。

これは検索すれば確認できますので、このプロジェクトで実施します。

### 方法
「この世の中のあらゆるすべての整数列」を、円周率データから総当り・逐次検索するプログラムを作成します。  

### プログラムの方針
円周率データ「Π」も、この世の中のあらゆるすべての整数列「n」も（おそらく）無限です。  
Πに出現したnをすべて記録することは不可能ですし、個人プロジェクトであるこの実行環境は非力なものとなりますので、


> [!NOTE]
> [プロジェクト稼働環境](https://genhirano.coresv.com/)では現在、円周率データ22,600,000,000,000桁(22兆桁)を対象に実行されています。このデータは約9TBです。
