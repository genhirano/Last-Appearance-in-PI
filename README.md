...
* YCD file format reading is modularized for user-friendly use（YCDファイル読み込みが使いやすくモジュール化されている）
  * "The 'YCD' file format, which contains a vast amount of stored digits of pi, is compressed for storage efficiency. While this format is convenient for storage, it can be inconvenient when attempting to directly use it from a program. In this project, the reading portion is modularized clearly to enable direct handling of 'YCD' format files generated by the 'y-cruncher' program in Java. The 'YCD_SeqProvider' class is designed to provide digits in specified increments and is a user-friendly module that can be used with an enhanced for loop."
  * 膨大な円周率が格納されている”YCD”形式ファイルは保管効率のために圧縮されています。保管には都合の良いこの形式もプログラムから直接利用しようとしたときに不便なこともあります。このプロジェクトでは、"[y-cruncher](http://www.numberworld.org/y-cruncher/)プログラムが生成する”YCD”形式ファイルをJavadで直接扱えるように、読み込み部分を明確にモジュール化しています。YCD_SeqProviderクラスは、指定した桁数ずつ切り出して提供できるように設計され、拡張forループが利用可能なユーザーフレンドリーなモジュールです。
* An embedded web server is included.（組み込みWEBサーバー）
  ...
  * プログラムには組み込みWEBサーバーが実装されており、別にWEBサーバーを建てなくても、計算を開始するだけでWEB画面により計算中の様子が確認できます。
